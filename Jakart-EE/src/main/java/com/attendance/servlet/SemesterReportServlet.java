...
public class SemesterReportServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        String search = req.getParameter("studentSearch");
        ...
        List<AttendanceSummary> report = selectedClassId > 0
                ? AttendanceRecordDAO.getSemesterReport(teacherId, selectedClassId, search)
                : List.of();
        req.setAttribute("classes", classes);
        req.setAttribute("selectedClassId", selectedClassId);
        req.setAttribute("report", report);
        req.setAttribute("search", search);
        req.getRequestDispatcher("/semester-report.jsp").forward(req, resp);
    }
